...
fn main() {
    ...
    let loc = block_on(loc::analyze_loc(path.clone())).unwrap();
    println!("LOC: {} files", loc.len());
    for f in loc.iter().take(3) {
        println!("  {:>5} loc  {}", f.code, f.path);
    }

    let hs = block_on(hotspots::analyze_hotspots(path.clone())).unwrap();
    ...
    let ch = block_on(churn::analyze_churn(path.clone(), 3650)).unwrap();
    println!("\nChurn: {} file·weeks", ch.len());
    for c in ch.iter().take(3) {
        println!("  {} {}  +{} -{}", c.week, c.path, c.added, c.deleted);
    }

    let cp = block_on(coupling::analyze_coupling(path.clone())).unwrap();
    println!("\nCoupling: {} pairs", cp.len());
    for p in cp.iter().take(3) {
        println!("  {}x  {}  <->  {}", p.count, p.file_a, p.file_b);
    }

    let ow = block_on(ownership::analyze_ownership(path)).unwrap();
