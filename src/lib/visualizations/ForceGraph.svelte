<script>
  ...
  function draw() {
    ...
    // nodes from unique endpoints, weighted by total coupling
    const nodeMap = new Map();
    const bump = (id, c) => {
      const n = nodeMap.get(id) || { id, weight: 0 };
      n.weight += c;
      nodeMap.set(id, n);
    };
    for (const p of data) {
      bump(p.file_a, p.count);
      bump(p.file_b, p.count);
    }
    ...
    // color nodes by top-level directory
    ...
    const link = container
      ...
      .data(links)
      .join("line")
      .attr("stroke-width", (d) => thickness(d.count));
    ...
    node
      .filter((d) => radius(d.weight) > 8)
      .append("text")
      .attr("class", "node-label")
      .attr("x", (d) => radius(d.weight) + 3)
      .attr("dy", "0.32em")
    ...
    sim = d3
      ...
      .force(
        ...
        d3
          .forceLink(links)
          .id((d) => d.id)
          .distance((d) => 90 - thickness(d.count) * 6)
          .strength(0.4)
      )
      ...
      .force(
        "collide",
        d3.forceCollide().radius((d) => radius(d.weight) + 4)
      )
      .on("tick", () => {
        link
